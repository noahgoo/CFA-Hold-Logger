...
const URL = `https://firestore.googleapis.com/v1/projects/${REACT_APP_FIREBASE_PROJECT_ID}/databases/(default)/documents/hold_logs?key=${REACT_APP_FIREBASE_API_KEY}`;
...
for (const { p, h, m } of LOGS) {
  ...
  if (res.ok) {
    ok++;
    console.log(
      `  ✓  ${p.padEnd(20)} ${h}:${String(m).padStart(2, "0")} Hawaii`,
    );
  } else {
